...
class ModestDataLoader:
    """
    Loads all MODEST data needed for plotting:
    - Continuum image
    - Observed Stokes
    - Deconvolved Stokes
    - Fitted Stokes (inverted profiles)
    - Wavelength arrays
    - SPINOR 2D atmospheric parameters
    """
    def __init__(self, psf_path):
        self.modest_dir = Path("/scratchsan/observatorio/juagudeloo/data/hinode-MODEST/INV_560_AR11967/")
        self.psf_path = Path(psf_path)
        # Data containers
        self.continuum = None
        self.obs_stokes = None
        self.deconvolved_stokes = None
        self.inverted_profs = None
        self.inverted_atmos = None
        self.wl = None
        self.wl_inv = None
    ...
    def load_obs_stokes(self, filename="inverted_obs.1.fits"):
        path = self.modest_dir / filename
        with fits.open(path) as hdul:
            # Only Stokes I (0) and V (1)
            self.obs_stokes = hdul[0].data[:, :, [0,1], :]
    ...
    def deconvolve_stokes(self, noise_level=0.01):
        """
        Deconvolve Stokes data using Wiener deconvolution.
        """
        from scipy.fft import fft2, ifft2, fftshift
        stokes = self.obs_stokes
        psf = self.load_psf()
        ny, nx, n_stokes, n_wavelength = stokes.shape
        deconv = np.zeros_like(stokes)
        # Pad PSF to image size
        def wiener_deconv(image, psf, noise_level):
            psf_padded = np.zeros_like(image)
            pc = (psf.shape[0] // 2, psf.shape[1] // 2)
            ic = (image.shape[0] // 2, image.shape[1] // 2)
            y0, y1 = ic[0] - pc[0], ic[0] - pc[0] + psf.shape[0]
            x0, x1 = ic[1] - pc[1], ic[1] - pc[1] + psf.shape[1]
            psf_padded[y0:y1, x0:x1] = psf
        ...
        for wave_idx in range(n_wavelength):
            for stokes_idx in range(n_stokes):
                image_2d = stokes[:, :, stokes_idx, wave_idx]
                deconv[:, :, stokes_idx, wave_idx] = wiener_deconv(image_2d, psf, noise_level)
        self.deconvolved_stokes = deconv
        return self.deconvolved_stokes

    def load_inverted_profs(self, filename="inverted_profs.1.fits"):
        path = self.modest_dir / filename
        with fits.open(path) as hdul:
            # Only Stokes I (0) and V (1)
            self.inverted_profs = hdul[0].data[:, :, [0,1], :]
        return self.inverted_profs

    def load_inverted_atmos(self, filename="inverted_atmos.fits"):
        path = self.modest_dir / filename
        with fits.open(path) as hdul:
            self.inverted_atmos = hdul[0].data
        return self.inverted_atmos

    def get_wavelength_arrays(self):
        ...
        self.wl = wl * u.Angstrom
        # Inverted profiles: 250 points
        NAXIS1_inv = 250
        CRVAL1_inv = 6302.0
    ...
    def extract_region(self, y_start, y_end, x_start, x_end):
        region = {}
        region['continuum'] = self.continuum[y_start:y_end, x_start:x_end]
        region['obs_stokes'] = self.obs_stokes[y_start:y_end, x_start:x_end, :, :]
        if self.deconvolved_stokes is not None:
            region['deconvolved_stokes'] = self.deconvolved_stokes[y_start:y_end, x_start:x_end, :, :]
        region['inverted_profs'] = self.inverted_profs[y_start:y_end, x_start:x_end, :, :]
        region['inverted_atmos'] = self.inverted_atmos[:, y_start:y_end, x_start:x_end]
        return region

    def build_spinor_atm_dict(self, y_start, y_end, x_start, x_end):
        ...
        for i, tau in enumerate(tau_values):
            d[f"temperature_tau_{tau}"] = self.inverted_atmos[temp_indices[i], y_start:y_end, x_start:x_end]
            d[f"velocity_tau_{tau}"] = self.inverted_atmos[vel_indices[i], y_start:y_end, x_start:x_end]
            b_field = self.inverted_atmos[mag_field_indices[i], y_start:y_end, x_start:x_end]
            gamma_deg = self.inverted_atmos[gamma_indices[i], y_start:y_end, x_start:x_end]
    ...
    def load_all(self, region_bounds=None):
        ...
        if region_bounds:
            ...
            spinor_dict = self.build_spinor_atm_dict(y_start, y_end, x_start, x_end)
            return {
                "region": region,
                "wl": self.wl,
                "wl_inv": self.wl_inv,
                "spinor_atm_dict": spinor_dict
            }
        else:
            return {
