    ...
    for n_spec_points in test_spectral_res:
      ...
      for model_type in models_types:  
        ...
        if model_type == "linear":
          stokes_data_train =  np.reshape(np.copy(stokes_data[0]), (stokes_data[0].shape[0]*stokes_data[0].shape[1], stokes_data[0].shape[2]*stokes_data[0].shape[3]))
          model = LinearModel(n_spec_points*4,6*20,hidden_units=1024).to(device)
        elif model_type == "cnn1d_4channels":
          stokes_data_train =  np.reshape(np.copy(stokes_data[0]), (stokes_data[0].shape[0]*stokes_data[0].shape[1], stokes_data[0].shape[2],stokes_data[0].shape[3]))
          stokes_data_train =  np.moveaxis(stokes_data_train, 2, 1)
          model = CNN1DModel(4,6*20,hidden_units=1024, signal_length=n_spec_points).to(device)
        elif model_type == "cnn1d_36channels":
          stokes_data_train =  np.reshape(np.copy(stokes_data[0]), (stokes_data[0].shape[0]*stokes_data[0].shape[1], stokes_data[0].shape[2],stokes_data[0].shape[3]))
